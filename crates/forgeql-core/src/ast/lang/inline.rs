//! Test-only in-crate language implementations for C++ and Rust.
//!
//! The production implementations live in the `forgeql-lang-*` crates. These
//! inline duplicates are retained by design, not left behind: a handful of
//! `forgeql-core`'s own unit tests both parse real C++/Rust source and assert on
//! `pub(crate)` interning state (such as `SymbolTable::strings`), so they can
//! neither move to an integration test (that state is not public API) nor run on
//! a fake language stub (they need real parsing). Because a crate's own unit
//! tests cannot link a `forgeql-lang-*` plugin ("multiple different versions of
//! crate `forgeql_core`"), those tests build a [`super::LanguageRegistry`] from
//! these clones instead. `RustLanguageInline` is kept alongside
//! `CppLanguageInline`; the real plugins' behaviour is covered by the
