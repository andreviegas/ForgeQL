...
| Command | Returns |
|---|---|
| `FIND symbols` | All indexed AST nodes. Use `WHERE fql_kind = '...'` to narrow. Every row carries a stable `node_id`, and a real workspace-total `usages` count of `role = 'code'` sites on every row EXCEPT a local-scope variable, which carries no `usages` value at all — its name identifies nothing outside the block declaring it, and the engine does not resolve scoped references, so the number would be the corpus-wide count of the NAME (every `ret` in a C tree reporting the same figure). Such a row therefore ranks behind every row that has a count under `ORDER BY usages` in either direction, matches no `usages` predicate at all — `usages = 0` included, so the dead-code recipe no longer sweeps in every local binding — and renders an empty metric column rather than `0`. A function PARAMETER is the one local-looking row that still carries its name's count: the scope enricher writes `scope` only on the declaration kinds its language declares, so a row it never examined is not a row known to be local. `ORDER BY usages DESC` and `WHERE usages > N` work on everything else. An exact-name query that finds nothing but whose name *is* used somewhere returns a `hint` saying how many code usage sites exist, so "not declared here" and "not here at all" are distinguishable. |
...
> files hold — and the skip is counted in the `hint`, so a name living only in
> such a file answers zero with the response saying a candidate went unread.
> That is the indexed backend; the in-memory one reads no files at all, so it
> counts nothing and its zero carries no such signal.
> Everything else is decoded leniently, so a file that is text
...
> the same `hint`, under its own reason, rather than passed over in silence —
> as a count, not a list of
> paths, so the `hint` says how much is missing and why but not which file — while one
...
| Command | Returns |
|---|---|
| `SHOW body OF` | Source text of a symbol. **Default `DEPTH 0`**: signature only, body replaced by `{ ... }`. `DEPTH 1`+: progressively reveals nested structure. `DEPTH 99`: full source. In CSV output the first column is a **node-relative 1-based `off`set** (not an absolute line) and the node's id is in the header — so you can `CHANGE NODE '<id>'` straight from the read. Absolute line numbers are available in `format=JSON`. A `WHERE` here is split between the returned lines and the symbol lookup: the fields a line carries — `text`, `marker`, `line`, `node_id`, `rev` — filter the lines, and anything else scopes which symbol `OF` resolved to, so `WHERE language = 'rust'` picks the Rust definition of a name two languages share. `ORDER BY`, `GROUP BY` and `HAVING` are not split, and are refused unless they name a line field. |
| `SHOW signature OF` | Declaration line only (return type, name, parameters). It renders one line rather than a row set, so there is nothing here for a predicate to filter: its clause only scopes which symbol is resolved, and a field only a source line carries (`text`, `marker`, `rev`) is refused rather than accepted and ignored. |
...
| `SHOW COMMITS` | The commits this session's branch carries since its base, newest first. A commit row carries `hash` and `subject` and nothing else, so those are the only names its clauses may use; anything else is refused, naming them. It pages like the `FIND` verbs — `find_limit` rows when no `LIMIT` says otherwise — and its `total` is every commit the clauses matched, counted before `OFFSET` and `LIMIT` cut the page, so a `total` above the row count means commits the page did not show: older ones held back by a `LIMIT`, newer ones skipped by an `OFFSET`. |
...
all three want no uncommitted rows in the session, no two segments of the index
...
total. `IN` and `EXCLUDE` are welcome throughout — a segment is one
source path, so the globs select whole segments — and they narrow the reading as
well as the answer.

For a counted grouping — whichever of the three answered — counted or
...
| Clause | Purpose |
|---|---|
| `WHERE` | Filter rows. Repeatable (implicit AND); `AND` is an accepted synonym for a repeated `WHERE`. Works on all field types including dynamic and enrichment fields. A field **no row of any shape can answer** is refused wherever it is written — `node_kind`, and the names belonging to another verb's rows: `size`, `depth`, `extension`/`ext` (`FIND files`), `signature` (`SHOW signature`), `marker` (a `SHOW` line row), `declaration` (`SHOW members`). The refusal names the field and where it *is* answered; it is never a zero-row answer, because zero rows is a claim about the corpus and an error is a fact about the query. The same test reaches a **value** wherever the ENGINE, not the corpus, owns the set of values a field can take — `fql_kind`, whose vocabulary a language plugin maps its grammar onto rather than extends, and `role` on `FIND usages`, minted by the read pass that finds the site — so `WHERE fql_kind = 'impl'` is refused naming every accepted kind instead of matching nothing, while where the CORPUS owns the set the empty answer is the fact about the code and stays an answer, so `guard_kind = 'ifdef'` on a corpus holding no such guard still returns nothing rather than an error. The value check reads `=` and `!=` only, because a pattern names no value: `fql_kind LIKE '%_block'` is untouched. Beyond that the verbs divide by how many consumers the clause has. `FIND symbols`, `FIND globals`, `FIND usages`, `FIND files`, `SHOW outline`, `SHOW NODE`, `SHOW LINES`, `SHOW MORE`, `SHOW COMMITS` and `SHOW DIFF` resolve no symbol, so any other field their rows do not carry is refused too and the message lists the ones they do. `SHOW members`, `SHOW callees`, `FIND callees OF`, `SHOW body` and `SHOW context` also address a symbol, so their `WHERE` is **split between the two**: a predicate the returned rows carry filters those rows, and one they do not carry scopes the lookup instead — `SHOW members OF 'Foo' WHERE language = 'cpp'` picks the C++ `Foo` and answers with its members. No predicate is dropped; the one kind that reaches both is a field the rows carry whose value comes from the resolved symbol rather than the row, which today is `path` on a callees row (every call sits in the resolved function's own file, so it names *which* function rather than filtering the calls). `IN` and `EXCLUDE` scope the lookup the same way on the verbs whose rows have no file of their own — a members row and a source line — instead of filtering those rows to nothing. When nothing satisfies the lookup half the answer is `no symbol 'Foo' matches WHERE …` — a fact about the workspace, worded so it cannot be mistaken for a refusal. `SHOW signature` is the one verb with no row set at all: it renders a single line, so its clause can only scope the lookup and a field only a line row carries (`text`, `marker`, `rev`) is refused there. The one thing these verbs cannot tell you is that a name is unknown rather than unmatched: only the indexed `FIND` verbs see which enrichment columns exist, so on a `SHOW` a misspelt field reads as a lookup that matched nothing. |
| `HAVING` | Filter after `GROUP BY` aggregation. Operates on `count`, which the grouping pass writes — so `count` is answerable here and in `ORDER BY`, and refused in a `WHERE`, which runs before it. Refused entirely on the verbs that answer with source lines, which never group, alongside `ORDER BY` and `GROUP BY`. |
...
| `LIMIT` | Maximum rows returned. Implicit cap of 20 when omitted on `FIND` and on `SHOW COMMITS`. On `FIND usages` without `GROUP BY` the unit is **files**, not rows: the cap selects whole files and every site of a selected file is returned. |
...
| `fql_kind` | string | Universal kind: `function`, `class`, `struct`, `enum`, `variable`, `field`, and the rest. The engine owns this set — a language plugin maps its grammar onto these names rather than adding to them — so a kind outside it is **refused** naming every accepted one, not answered with a zero. One value has two spellings: a row nothing maps is stored with the empty kind and RENDERED by `SHOW outline` as `unknown`. Both are inside the set, both are served, and both answer the same rows and the same total — the predicate is spelled to the stored value before it reaches a reader, the empty kind has a posting like every other kind, and a row whose kind is empty reports it as the value it is, so a scan's `NOT MATCHES` keeps that row too. An empty answer on this field is a fact about the corpus, for these two values as for every other, **on the verbs whose rows carry a kind at all** — a `FIND usages` row is one line of one file and carries none, so this field is accepted there and matches nothing whichever spelling is written, exactly as it did before those two values were served. |
...
| `usages` | integer | Workspace-total count of **`role = 'code'` sites only**, aggregated from the reference index at index time — on every row EXCEPT a local-scope variable, which carries no `usages` value at all, because its name identifies nothing outside the block declaring it and the engine does not resolve scoped references, so the number would be the corpus-wide count of the NAME. `ORDER BY usages DESC` and `WHERE usages > N` are real queries, not heuristics, on every row that has a value; a row that has none matches NO `usages` predicate (`= 0` included, so the dead-code recipe no longer sweeps in every local binding), ranks behind every valued row under `ORDER BY usages` in either direction, and renders an empty metric column rather than `0`. A function PARAMETER is the one local-looking row that still carries its name's count — the scope enricher writes `scope` only on the declaration kinds its language declares, so a row it never examined is not a row known to be local. The count deliberately excludes every non-`code` occurrence — comment, string, build-file-argument and documentation-prose sites today — so it is smaller than the `total` of `FIND usages OF` for the same name: this column ranks symbols by how much code depends on them, not by how often the name is written. |
| `role` | string | On `FIND usages` rows only: what the name was written in — `code`, `comment`, `string`, `config`, `doc` and `text` (see the role table above). `WHERE role = 'code'` narrows to references the compiler resolves; `GROUP BY role` sizes a rename by kind. Those six, plus the empty string the renderer prints where a site carries no role at all, are the whole set — anything else is **refused** naming them rather than answered with a zero, the same treatment `fql_kind` gets and for the same reason: the engine owns this value set, not the corpus. `!=` is read the same way as `=`. The empty role only ever appears on the in-memory backend, which tags no site: there every `role = …` filter is empty, so an empty answer to one is a fact about the backend rather than about the corpus. |
...
|---|---|---|
| `name` | string | Symbol name |
| `fql_kind` / `kind` | string | Universal kind (e.g. `function`, `class`). A node no language maps is stored with no kind at all and **rendered here as `unknown`** — not as its raw tree-sitter name — so `WHERE fql_kind = 'unknown'` on an outline matches what the outline printed. On a symbol row the same node reads as the empty kind: `GROUP BY fql_kind` groups it under the empty name. Both spellings are accepted values and both select the same rows on every verb, this one included — an equality is spelled to the stored value before it reaches a reader, and the outline's rendered `unknown` is spelled to match it — so `WHERE fql_kind = ''` and `= 'unknown'` answer the same outline rows and the same `FIND` total. A `LIKE` or `MATCHES` carries a pattern rather than a value and is matched against the spelling the verb rendered. A kind that is neither is refused naming every accepted one, because the engine and not the corpus owns this set. Rows are printed under `fql_kind`; `kind` filters and sorts the same column. |
...
`SHOW outline` and `SHOW members` support the full clause pipeline including `WHERE`, over the fields their own rows carry — an outline row carries `name`, `fql_kind` (or `kind`), `path`/`file`, `line` and `depth`; a members row carries `fql_kind`/`kind`/`type`, `text`/`declaration`/`name` and `line`. `SHOW outline` names a file, so its whole clause only filters and anything outside that list is refused. `SHOW members` names a symbol, so its `WHERE` is split: the fields above filter the members, and a field they do not carry — `language`, say — scopes which symbol `OF` resolved to. Its `ORDER BY`, `GROUP BY` and `HAVING` are not split and are held to the list above:
