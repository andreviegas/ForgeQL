...
| `IN` | Restrict to files matching glob pattern. On a verb whose rows carry no file of their own — `SHOW members`, and the verbs that return source lines — it scopes which symbol was resolved rather than filtering those rows, which it could only ever empty. |
