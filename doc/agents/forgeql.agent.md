...
| Rewritten outside ForgeQL | A file a formatter, build step or editor rewrote is re-indexed by the next command that names a node, a symbol or the file (`SHOW NODE`/`LINES`/`outline` of a file or handle — a directory or glob outline lists many files and is not checked — `body`/`context`/`signature`/`callees`/`members`, `FIND NODE`, every node mutation, the `FOUND` sweeps), which answers the current lines (a read or `FIND NODE` adds a `hint` saying so; a mutation carries none, its boundary diff shows them, and a rev-less form — an EOF append, `COPY NODE … TO`, `COPY NODES FOUND TO` — is checked the same way but has nothing to refuse, so it writes from the re-indexed bytes unannounced); a rev from before the rewrite is refused where the node's bytes changed, and an unchanged node is edited where it now is. One repeat of the lookup the verb itself runs (a gated command resolves its symbol or handle twice, same index tier, nothing newly scanned, and once more per file it re-indexes — the naming repeats until a pass re-indexes nothing), one `stat` per named file, a hash when its size or mtime moved, and a second uncached hash when the answer maps lines to handles; a rewrite keeping both is not seen until either moves, `FIND` rows are not checked until the file is read or edited, `UNDO`/`ROLLBACK` restore whole files from ForgeQL's own snapshots unchecked, the check reaches only what the stale index can still name (a symbol the rewrite introduced or renamed is "no symbol matches" until the file is read by handle or path; a file created outside ForgeQL stays unindexed until ForgeQL itself writes it or the next attach rebuilds the index — a reconnect re-indexes only tracked files `git diff HEAD` lists; a deleted or unreadable one keeps its rows until ForgeQL next writes or re-indexes it, is never taken as verified, and reads of it fail on the missing bytes; a re-index that fails refuses the command with its reason), and the in-memory backend (no `.forgeql.yaml`) stores no content id and is not covered at all |
...
`fql_kind` is the language-agnostic kind field, and `kind` is its alias, answered wherever `fql_kind` is. Raw `node_kind` values (tree-sitter grammar names) are language-specific, and no row of the indexed backend every session queries stores them — so `WHERE`, `ORDER BY` and `GROUP BY` on `node_kind` are **refused** on every verb that filters rows (`FIND symbols`, `FIND globals`, `FIND usages`, `FIND files`, `FIND callees OF`, `SHOW outline`, `SHOW members`, `SHOW callees`), rather than reporting a confident absence. Use `fql_kind`. A `fql_kind` VALUE outside the engine's vocabulary is refused too, and for the same reason: `WHERE fql_kind = 'impl'` (Rust `impl` blocks are `class`) fails naming every accepted kind rather than answering a silent zero. `!=` is refused too, and there the old answer was not a zero but EVERYTHING: `fql_kind != 'impl'` excluded nothing and returned every row, an inert predicate that reads as a filtered result. That is the one shape where a complete answer became a refusal, and it is deliberate — so the refusal, not this table, is the complete list; the table below is the working subset. `role` on `FIND usages` (`code`, `comment`, `string`, `config`, `doc`, `text`, and the empty string the renderer prints where a site carries no role at all) is the other set the ENGINE owns and behaves the same way. A field whose values the CORPUS owns keeps its empty answer: `guard_kind = 'ifdef'` on a corpus holding no such guard returns nothing, and nothing is the correct answer. One value has two spellings: a row nothing maps is stored with the empty kind and RENDERED by `SHOW outline` as `unknown`. Both are INSIDE the accepted set, both are served, and both select the same rows and report the same total on every verb — the equality is spelled to the stored value before it reaches a reader, the empty kind carries a posting like every other kind, and on a scan a row with no kind compares as the empty string rather than as a missing field, so `NOT MATCHES` keeps it as well. A `LIKE` or `MATCHES` carries a pattern rather than a value and is matched against the spelling the verb rendered. An empty answer on this field is a fact about the corpus, for these two values as for every other, on the verbs whose rows carry a kind at all — a `FIND usages` row is one line of one file and carries none, so this field is accepted there and matches nothing whichever spelling is written, exactly as it did before those two values were served.
