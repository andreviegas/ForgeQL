...
- A `FIND symbols` scan that would materialise past the query's 2 GiB result budget (~1.3M rows) is **refused, not truncated** — narrow with `IN`/`WHERE`, or use `ORDER BY <field> LIMIT k` (k ≤ 1000, no `OFFSET`, no `GROUP BY`, no `HAVING`): a running top-K trim keeps that form under the budget while still scanning every segment, so the answer is the true top K, a full page of it, and a `total` counting every row that matched — the trim reports what it sheds, and duplicates on `name`, `fql_kind`, path and line are collapsed before anything sheds. A `HAVING` is excluded because it runs after the page is cut, so such a query is refused by this budget rather than answered wrongly. Two forms still stop reading early and can only report what they read, both known defects pinned as failing golden cases: a bare `LIMIT` with no `ORDER BY` truncates the scan rather than paging it, so `OFFSET` skips rows that were never fetched, duplicates collapsing afterwards can shorten the page, and its `total` is the row count returned; and `ORDER BY name` with a small `LIMIT` streams that many rows out of the name index and reports that many as its `total`. Everywhere else `FIND symbols` reports how many rows matched, as `FIND usages` always has. The bound covers that scan over the on-disk index; `FIND usages`, `FIND files` and the in-memory backend carry none.
