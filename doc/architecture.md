...
The `WHERE` predicate supports `=`, `!=`, `LIKE`, `NOT LIKE`, `MATCHES`, `NOT MATCHES` (regex via the `regex` crate), and numeric comparisons. `ClauseTarget` is implemented for every result shape — `IndexRow`, `SymbolMatch`, `FileEntry`, `DiffFileEntry`, `OutlineEntry`, `MemberEntry`, `SourceLine` and `CallGraphEntry` — so the full pipeline applies uniformly to FIND queries, SHOW body/lines/context, SHOW outline/members/callees and SHOW DIFF. Each implementation also *declares* the field names it resolves (`STR_FIELDS`, `NUM_FIELDS`) and whether anything outside them can resolve at all (`OPEN_FIELDS`), and `filter::reject_unresolvable_fields::<T>` refuses a `WHERE`, `ORDER BY`, `GROUP BY` or `HAVING` naming a field shape `T` does not carry, rather than running the filter and returning an empty answer. The verbs split in one way that matters. `FIND files`, `SHOW outline`, `SHOW MORE` and `SHOW DIFF` resolve nothing, so their clause is checked outright; `FIND symbols`/`globals`/`usages` are checked the same way but in the columnar backend's Stage 0, which alone can see the enrichment columns a segment stored, because a symbol row's field map is open. The rest — `SHOW members`, `SHOW callees`, `FIND callees OF`, `SHOW body`, `SHOW context`, `SHOW NODE` — use one clause twice: the storage engine applies it to symbol rows to decide which symbol `OF` names, and it then reaches the rows that came back. Those predicates are partitioned out by `filter::without_resolve_only_predicates::<T>` before the row filter runs, because re-applying them drops every row — `SHOW members OF 'Foo' WHERE language = 'cpp'` resolved the right type and then returned none of its members. What remains is checked against the row shape like any other. `SHOW signature` is outside all of it: it renders one line and applies no `WHERE`. The clause matters as much as the field: grouping keys a row through `field_str` alone, so a numeric-only name would fabricate one empty-named group holding every row, and `count` is written by the grouping pass, so it is answerable in `HAVING` and `ORDER BY` and refused in a `WHERE` that runs before it.
