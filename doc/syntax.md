...
| `role` | string | On `FIND usages` rows only: what the name was written in — `code`, `comment`, `string`, `config`, `doc` and `text` (see the role table above). `WHERE role = 'code'` narrows to references the compiler resolves; `GROUP BY role` sizes a rename by kind. Those six are the whole set the engine can mint, so a seventh is **refused** naming them rather than answered with a zero — the same treatment `fql_kind` gets, and for the same reason: the engine owns this value set, not the corpus. |
