...
so the enrichment version stays at 73 and the overlay schema version at 17.

**Fixes a crash that closed the connection with nothing sent.** Reading into
a region an out-of-band rewrite had shifted made a span subtraction
underflow, which panics under overflow checks, and the HTTP server did not
catch it — so an agent saw a dead socket rather than an error. Both the
panic's cause and its containment are fixed. Upgrading is worth doing for
this alone.
...
## [0.193.0] — 2026-08-29 — the kindless `fql_kind` answers its own equality

**Rebuilds overlays once, no re-index:** the overlay schema version moves to
17 and the enrichment version is unchanged at 73. Segments already carried
the empty kind; only the merge that builds an overlay from them dropped it,
so overlays are rebuilt on first use and segments are reused as they are.

**Behaviour change worth reading before upgrading:** a `.forgeql.yaml` that
exists and does not parse now refuses the session instead of being read as
no configuration at all. A source that has been running degraded on a
malformed file will start refusing, and the refusal names the file and the
parse error. Absence of the file is unchanged.
...
- `WHERE fql_kind = ''` now answers the rows `GROUP BY fql_kind` counts under
  ...
  The in-memory backend kept its own copy of the builder's skip, in the
  secondary index it consults for an equality; that mattered more than a
  smaller index, because once the index has supplied the candidates the
  predicate is stripped, so an unindexed value reached no scan that could
  have decided it and answered 0 rows with a success status. It is indexed
  and counted like any other value now, and a source with no configuration
  file answers these two the same way a fully indexed one does.    The
  overlay schema version moves to 17. There is no layout change and no   re-
  indexing: overlays built by an older binary are missing the new posting,
  so   they are rebuilt on first use. `SHOW outline … WHERE fql_kind =
  'unknown'`,   which already worked, still answers the same rows.

## [0.192.0] — 2026-08-29 — the stamp-only booleans answer their `'false'` value

**No re-index:** nothing stored changes, so the enrichment version stays at
the 73 the previous release moved it to and existing segments stay valid. The
value below is computed from what the index already holds.
...
## [0.191.0] — 2026-08-26 — a marker comment that opens a function body is counted

**Re-indexes once on upgrade:** the enrichment version moves to 73, so every
corpus rebuilds its segments on first use after this release. A segment from
the previous version is not merely missing rows — a function that already
carried a marker elsewhere records a smaller `todo_count` there, and can be
missing a tag.
...
- Each of the three exclusions above is now pinned by a test in all four
  registered grammars. The doc-comment exclusion had none: the sentence
  asserting it appeared in five documentation files with nothing that fails
  when it stops being true. Verified by mutation rather than by observing a
  pass — teaching the scan to read the preceding sibling turns six tests red.
- A file-level count in the suite claimed a pre-change baseline of 8 rows and
  a gain of four. Measured against the released engine the baseline is 11 and
  the gain is one, the same row a neighbouring case already pins; the earlier
  number came from a superseded enrichment version. The count it asserts was
  always right, and it no longer claims breadth it cannot show.
...
- The refusal glosses the empty value with the field it is talking about, so
  a rejected `role` value is no longer answered with a sentence about kinds.

- The outline's rendering of a kindless row is pinned on both backends. It was
  the one behaviour in this change reachable only through the in-memory one,
  and so the only part of it a test could not have caught.
