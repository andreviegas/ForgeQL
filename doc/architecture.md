...
**No rows at all.** `SHOW signature` renders a single line rather than building a row set, so nothing in its answer can be filtered and its clause can only scope the lookup. A field only a source-line row carries — `text`, `marker`, `rev`, derived as what `SourceLine` resolves minus what a symbol row resolves — is refused there outright rather than accepted and quietly discarded.
