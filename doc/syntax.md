...
| Command | Returns |
|---|---|
| `SHOW body OF` | Source text of a symbol. **Default `DEPTH 0`**: signature only, body replaced by `{ ... }`. `DEPTH 1`+: progressively reveals nested structure. `DEPTH 99`: full source. In CSV output the first column is a **node-relative 1-based `off`set** (not an absolute line) and the node's id is in the header — so you can `CHANGE NODE '<id>'` straight from the read. Absolute line numbers are available in `format=JSON`. A `WHERE` here filters the returned lines, so use the fields a line carries — `text`, `marker`, `line`, `node_id`, `rev`. Anything else is applied to the lines too and matches none of them: `WHERE language = 'rust'` returns an empty body rather than disambiguating the symbol (known defect — see the `WHERE` row in Universal Clauses). |
...
| Clause | Purpose |
|---|---|
| `WHERE` | Filter rows. Repeatable (implicit AND); `AND` is an accepted synonym for a repeated `WHERE`. Works on all field types including dynamic and enrichment fields. A field **no row of any shape can answer** is refused wherever it is written — `node_kind`, and the names belonging to another verb's rows: `size`, `depth`, `extension`/`ext` (`FIND files`), `signature` (`SHOW signature`), `marker` (a `SHOW` line row), `declaration` (`SHOW members`). The refusal names the field and where it *is* answered; it is never a zero-row answer, because zero rows is a claim about the corpus and an error is a fact about the query. Beyond that the verbs split. `FIND symbols`, `FIND globals`, `FIND usages`, `FIND files`, `SHOW outline`, `SHOW MORE` and `SHOW DIFF` resolve nothing, so any other field their rows do not carry is refused too, and the message lists the ones they do. `SHOW members`, `SHOW callees`, `FIND callees OF`, `SHOW body`, `SHOW context` and `SHOW NODE` hand the same clause to the symbol lookup as well, so they are not gated that way — **and on the indexed backend the lookup does not evaluate it either, so a predicate their rows cannot carry (`WHERE language = 'cpp'` on `SHOW members`) scopes nothing and filters every row away. Known defect; filter those verbs on the fields their own rows carry.** `SHOW signature` renders one line and applies no `WHERE` at all. |
