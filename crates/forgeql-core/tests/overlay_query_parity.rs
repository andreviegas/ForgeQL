//! Overlay/columnar parity: query surface.
//!
//! `find_symbols` over a committed multi-segment overlay must return the same
//! rows as the merged legacy tables — across kind prefilter, exact-name and
//! LIKE filters, ORDER BY, enrichment-field filters, and multi-segment name
//! lookup.
