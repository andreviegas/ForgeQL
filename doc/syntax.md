...
| Command | Returns |
|---|---|
| `SHOW body OF` | Source text of a symbol. **Default `DEPTH 0`**: signature only, body replaced by `{ ... }`. `DEPTH 1`+: progressively reveals nested structure. `DEPTH 99`: full source. In CSV output the first column is a **node-relative 1-based `off`set** (not an absolute line) and the node's id is in the header — so you can `CHANGE NODE '<id>'` straight from the read. Absolute line numbers are available in `format=JSON`. A `WHERE` here does two jobs: it picks which symbol to resolve (`WHERE language = 'rust'` disambiguates a name two languages both define) and it filters the returned lines (`WHERE text MATCHES '…'`). The first kind is applied to the symbol lookup only — a source line carries no `language`, so re-applying it would drop every line of the body it just found — and anything neither kind recognises is refused. |
...
| Clause | Purpose |
|---|---|
| `WHERE` | Filter rows. Repeatable (implicit AND); `AND` is an accepted synonym for a repeated `WHERE`. Works on all field types including dynamic and enrichment fields. A field **no row of any shape can answer** is refused wherever it is written — `node_kind`, and the names belonging to another verb's rows: `size`, `depth`, `extension`/`ext` (`FIND files`), `signature` (`SHOW signature`), `marker` (a `SHOW` line row), `declaration` (`SHOW members`). The refusal names the field and where it *is* answered; it is never a zero-row answer, because zero rows is a claim about the corpus and an error is a fact about the query. Beyond that the verbs split. `FIND symbols`, `FIND globals`, `FIND usages`, `FIND files`, `SHOW outline`, `SHOW MORE` and `SHOW DIFF` resolve nothing, so any other field their rows do not carry is refused too, and the message lists the ones they do. On `SHOW members`, `SHOW callees`, `FIND callees OF`, `SHOW body`, `SHOW context` and `SHOW NODE` the same clause also picks **which symbol to resolve** — `SHOW members OF 'Foo' WHERE language = 'cpp'` disambiguates a type two languages both define — so a predicate the symbol lookup used is dropped before the row filter runs instead of emptying the result, and only what is left is checked against the row. `SHOW signature` renders one line and applies no `WHERE` at all. |
...
`SHOW outline` and `SHOW members` support the full clause pipeline including `WHERE`, over the fields their own rows carry — an outline row carries `name`, `fql_kind` (or `kind`), `path`/`file`, `line` and `depth`; a members row carries `fql_kind`/`kind`/`type`, `text`/`declaration`/`name` and `line`. `SHOW outline` refuses anything else outright, because its clause only filters. `SHOW members` refuses only what no row of any shape can answer, because its clause also picks which type to resolve — an enrichment field there scopes the lookup and is then dropped from the row filter rather than emptying it. Enrichment fields belong to `FIND symbols`, whose rows carry them:
