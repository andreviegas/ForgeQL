...
- A `FIND symbols` scan that would materialise past the query's 2 GiB result budget (~1.3M rows) is **refused, not truncated** — narrow with `IN`/`WHERE`, or add `LIMIT k` (k ≤ 1000, no `OFFSET`, no `GROUP BY`, no `HAVING`), with or without an `ORDER BY`: a running top-K trim keeps that form under the budget while still scanning every segment, so the answer is the true top k, a full page of it, and a `total` counting every row that matched — the trim reports what it sheds, and duplicates on `name`, `fql_kind`, path and line are collapsed before anything sheds. With no `ORDER BY` the ordering is the `(name, line, path, fql_kind)` tie-break the pipeline sorts by anyway, so `LIMIT k` returns the k smallest rows under it rather than the first k the scan happened to reach. A `HAVING` is excluded because it runs after the page is cut, so such a query is refused by this budget rather than answered wrongly; so is a scan of an index holding two segments built from one source path, where a segment collapsing its own duplicates is no longer the whole collapse. One form still stops reading early and can only report what it read, a known defect pinned as a failing golden case: `ORDER BY name` with a small `LIMIT`, no `IN`/`EXCLUDE` and no uncommitted edits streams that many rows out of the name index and reports that many as its `total`. Everywhere else `FIND symbols` reports how many rows matched, as `FIND usages` always has. The bound also covers the union of a session's uncommitted rows into that scan, the `FIND usages` site list on both backends (checked between matching tiers, so the peak can overshoot it by one tier's finds), and the in-memory backend's scan, trimmed or not — an armed trim's window is a few multiples of the LIMIT and counts against the same budget; `FIND files` carries none — one small row per workspace file, so its size is the file count, never what a query matches. The candidate row IDs the scan holds before building anything have a separate and much larger bound (~537M, four bytes each rather than 1,600) — `FORGEQL_FIND_MAX_ROW_IDS` overrides it, `0` disables it.
