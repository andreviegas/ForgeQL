...
- **Phase 5: FQOV v7 Global Enrichment Bitmaps** — Upgraded the overlay format to schema version 7 (TOC count: 11). A new `enrich_bitmaps` blob stores `RoaringBitmap`s keyed by `"field=value"` for all enrichment attributes, built at overlay-write time by `overlay_builder.rs` (step 5.5). `prefilter_global` in `columnar_storage.rs` now intersects enrichment bitmaps for Eq/Bool/Gte/Gt/Lte/Lt predicates, shrinking the candidate set from 37k+ rows to ~50–500 rows before segment materialisation. The path-filter fast-path (`has_any_indexed_predicate`) is also enrichment-aware. Numeric fields use lexicographic-scan + parse in `prefilter_enrichment_ge/le`; string/bool fields use exact key lookup in `prefilter_enrichment_eq`. The `group_by_file_fast_path_eligible` check intentionally excludes enrichment predicates (the fast path returns empty `name` fields, breaking sort order); enrichment-predicate GROUP BY file queries fall through to the normal pipeline, which is now fast via the enrichment-bitmap candidate narrowing.
- **Phase 4: The `index_files` Table in Overlay (FQOV v6)** — Upgraded the ForgeQL Columnar Overlay file format to schema version 6 and increased the table-of-contents TOC count to 10 slots. Serialised a raw flat array of `u32` file sizes (`index_files_bytes`) alongside segment metadata, eliminating expensive disk-based directory walks/metadata checks for file-system query acceleration. Added structural reader/writer bindings, automated version up-conversion, and robust runtime validations.
