...
That ordering is also what makes the reported `total` mean anything. `total` is the number of rows that matched, taken before `OFFSET` and `LIMIT` cut a page out of it, and the trim hands back how many rows it discarded so those are counted too — they are answers no page could hold rather than rows about to merge into a survivor. Two paths still report the size of the page instead, both because they stop reading and nothing counts what was never read: a bare `LIMIT` with no `ORDER BY`, which stops opening segments at `limit + 1`, and `ORDER BY name` with a small `LIMIT`, which streams that many rows out of the name FST — counting the rest there means walking the whole FST, which is the walk the stream exists to avoid. Both are enforced as failing cases in the golden suite rather than described anywhere as a caveat.
