//! Parser tests for the mutation verbs: `CHANGE` in each of its target forms,
//! the heredoc payload syntax, and `UNDO`.
//!
//! Two rejection tests sit here rather than with a statement family, because
//! what each one rejects is a mutation: `parse_top_level_rename_is_rejected`
//! covers the top-level `RENAME` removed in v0.10.0, and
//! `parse_error_missing_quote` an unterminated quote in a `CHANGE FILE`.
