...
| `IN` | Restrict to files matching glob pattern. On a verb that names a symbol and whose rows carry no file of their own — `SHOW members`, `SHOW body`, `SHOW context`, `SHOW signature` — it scopes which symbol was resolved rather than filtering those rows, which it could only ever empty. `SHOW NODE`, `SHOW LINES` and `SHOW MORE` also return rows with no path, but they resolve no name either, so a glob has nothing to scope and nothing to match and is refused there. |
