...
The `WHERE` predicate supports `=`, `!=`, `LIKE`, `NOT LIKE`, `MATCHES`, `NOT MATCHES` (regex via the `regex` crate), and numeric comparisons. `ClauseTarget` is implemented for every result shape — `IndexRow`, `SymbolMatch`, `FileEntry`, `DiffFileEntry`, `OutlineEntry`, `MemberEntry`, `SourceLine` and `CallGraphEntry` — so the full pipeline applies uniformly to FIND queries, SHOW body/lines/context, SHOW outline/members/callees and SHOW DIFF. Each implementation also *declares* the field names it resolves (`STR_FIELDS`, `NUM_FIELDS`) and whether anything outside them can resolve at all (`OPEN_FIELDS`), and there are two validations built on that. `filter::reject_refused_fields` refuses what **no** row of any shape can answer — read straight from `field_tiers`, so a name added there is refused everywhere without a second edit — and it runs on every verb with a clause. `filter::reject_unresolvable_fields::<T>` is the stricter check, refusing any field shape `T` does not carry, and it runs only where the clause *only* filters: `FIND files`, `SHOW outline`, `SHOW DIFF`, and (index-aware, in the columnar backend's Stage 0, because a symbol row's enrichment map is open) `FIND symbols`/`globals`/`usages`. It deliberately does not run on `SHOW members`, `SHOW callees` or the reading verbs: their clause also selects which symbol to resolve, so `SHOW members OF 'Foo' WHERE language = 'cpp'` is a working query that a members row cannot carry. The clause matters as much as the field: grouping keys a row through `field_str` alone, so a numeric-only name would fabricate one empty-named group holding every row, and `count` is written by the grouping pass, so it is answerable in `HAVING` and `ORDER BY` and refused in a `WHERE` that runs before it.
