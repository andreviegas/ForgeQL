//! Integration tests for the read/query surface — `FIND` and `SHOW`.
//!
//! Split out of `engine_integration`: `FIND symbols` / `FIND usages` /
//! `FIND globals` / `FIND variables`, and `SHOW body` / `SHOW context` /
//! `SHOW outline` / `SHOW members` — including their filters, ordering,
//! pagination, projection, and result-shape guarantees.
//!
//! Run with: `cargo test -p forgeql-core --test engine_queries`
#![allow(
    clippy::unwrap_used,
    clippy::expect_used,
