...
| Kconfig | `Kconfig` (any casing, no extension), `*.kconfig` | `config X` / `menuconfig X` as `variable` named `X` — the definition site of a build flag. Every flag reference is a usage site, so `depends on X`, `select X` and `if X` all answer `FIND usages OF 'X'`. `menu` and `if` are not named, so they carry no row: a Kconfig file's default `SHOW outline` is therefore empty, and `SHOW outline … ALL` is how you see its shape. Vendor-suffixed files (`Kconfig.stm32`) are not claimed — the suffix is open-ended. |
