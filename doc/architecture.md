...
- `node_kind` — the raw tree-sitter node kind (e.g. `function_definition`, `class_specifier`). Language-specific, computed while parsing to drive kind mapping, and stored on no row of the columnar index: a `WHERE` or `ORDER BY` on it is **refused** there, because a predicate on a field no row carries can only report absence. The legacy in-memory index does keep it per row and still filters on it, which is why the name remains in `CORE_WHERE_FIELDS` — the refusal lives in the columnar backend's own validation, next to the knowledge of what that backend stores.
- `fql_kind` — a universal kind (e.g. `function`, `class`, `struct`). Language-agnostic, defined by `map_kind()`.

Always use `WHERE fql_kind = 'function'`. Every session serves queries from the columnar index, so a query naming `node_kind` errors with a pointer to `fql_kind` rather than answering a confident zero. Serving it there would mean storing it, which is an index-output change; until something does, the refusal is the honest answer.
