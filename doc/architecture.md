...
The `WHERE` predicate supports `=`, `!=`, `LIKE`, `NOT LIKE`, `MATCHES`, `NOT MATCHES` (regex via the `regex` crate), and numeric comparisons. `ClauseTarget` is implemented for every result shape — `IndexRow`, `SymbolMatch`, `FileEntry`, `DiffFileEntry`, `OutlineEntry`, `MemberEntry`, `SourceLine` and `CallGraphEntry` — so the full pipeline applies uniformly to FIND queries, SHOW body/lines/context, SHOW outline/members/callees and SHOW DIFF. Each implementation also *declares* the field names it resolves (`STR_FIELDS`, `NUM_FIELDS`) and whether anything outside them can resolve at all (`OPEN_FIELDS`). Two validations are built on that. `filter::reject_refused_fields::<T>` refuses what **no** row of any shape can answer — read straight from `field_tiers`, minus what shape `T` itself carries — and runs on every verb with a clause except `SHOW signature`, which renders one line and applies none. `filter::reject_unresolvable_fields::<T>` is the stricter check, refusing any field `T` does not carry, and it runs only where the clause is *only* a row filter: `FIND files`, `SHOW outline`, `SHOW MORE`, `SHOW DIFF`, and (index-aware, in the columnar backend's Stage 0, because a symbol row's enrichment map is open) `FIND symbols`/`globals`/`usages`. It deliberately does not run on `SHOW members`, `SHOW callees` or the reading verbs, whose clause is also handed to the symbol lookup, so `SHOW members OF 'Foo' WHERE language = 'cpp'` is a query the legacy backend answers by scoping the lookup. **Known defect, pinned by a test:** the columnar resolver evaluates none of those predicates (only posted-enrichment `=`), so on the backend every session actually uses the predicate scopes nothing and is then applied to rows that cannot carry it, emptying the result for a symbol that exists — the fix is per-candidate predicate evaluation in `query/resolve.rs`, as `storage/legacy/resolve.rs` already does. The clause matters as much as the field: grouping keys a row through `field_str` alone, so a numeric-only name would fabricate one empty-named group holding every row, and `count` is written by the grouping pass, so it is answerable in `HAVING` and `ORDER BY` and refused in a `WHERE` that runs before it.
