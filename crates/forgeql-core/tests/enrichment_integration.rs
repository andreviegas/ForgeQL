...
///
/// The prefix property is unconditional now that the tie-break ends in
/// `fql_kind`: the four tie-breakers are the duplicate-collapse key, so the
/// ordering separates every pair of rows the answer tells apart, and the
/// bounded partition's instability has nothing left to choose between.
#[test]
