...
| Blast radius | `FIND usages OF 'name' GROUP BY file ORDER BY count DESC` — one row per occurrence site, includes non-call references. Every row carries a `role` (`code` for a resolved identifier, `comment` and `string` for the name written in comment text or a string literal, `config` for a build- or config-file value (never a key), `doc` for documentation prose, `text` for a site found by reading the file rather than by a posting): add `WHERE role = 'code'` for references the compiler sees, `GROUP BY role` to size the campaign by kind. Non-code roles are a review queue — read before sweeping. Unfiltered, **`LIMIT` counts files** (at most: a size ceiling can withhold further whole files from the tail and says so in a `hint` — then page with `OFFSET`, or narrow with `IN`/`WHERE`, not a larger `LIMIT`) and every site of a selected file is returned; `total` is the true site count. Each row carries its **file's** `node_id` and `rev`, so a site is editable where you read it. A name that is not identifier-shaped is reached two further ways, and both answer at once: as a **substring** of a stored whole token (`net/core/`, `zephyr/pm/device.h` — that is how an include path finds the files that include it), and by splitting it at its non-identifier characters and confirming the whole name against the source line at every site those parts propose — so `foo-bar.frozen`, which no language stored whole, is found anyway, and a name one language stored whole while another split it comes back as both sets merged. Nothing caps how many candidates get read: a name whose every part is common is slow on a large corpus and still complete, and one no part of which could ever be a token (`a.b`) is answered by reading the files |
