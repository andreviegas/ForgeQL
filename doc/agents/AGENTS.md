...
| Need | Command |
|---|---|
| Find a symbol | `FIND symbols WHERE name LIKE 'pattern' [WHERE fql_kind = '...']` — a kind outside the engine's vocabulary is **refused** naming every accepted one, not answered with a zero (`fql_kind = 'impl'` fails; Rust impl blocks are `class`). The same for `role` on `FIND usages`. A field whose values the CORPUS owns keeps its empty answer, so `guard_kind = 'ifdef'` on a corpus with no such guard still returns nothing |
