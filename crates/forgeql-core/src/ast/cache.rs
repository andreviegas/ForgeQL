...
///   32. Usage postings (`usages_fst`/`usages_postings` segment blobs) —
///       identifier text → source lines, the reference index.
///   33. ECUC parameter/reference values (XML family) named by their
///       DEFINITION-REF's last path segment instead of the bare tag name.
///
