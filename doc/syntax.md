...
| `SHOW signature OF` | Declaration line only (return type, name, parameters). Its `WHERE` is split like `SHOW body`: a line field filters the rendered line, anything else scopes which symbol is resolved. |
